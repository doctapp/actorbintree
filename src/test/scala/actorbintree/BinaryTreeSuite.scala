...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  test("Garbage collection with 3 elements (three nodes)") {
    val ts = system.actorOf(Props[BinaryTreeSet])
    for(i <- 0 to 2) {
      ts ! Insert(testActor, i, i)
      expectMsg(OperationFinished(i))
    }

    for(i <- 0 to 2) {
      ts ! Contains(testActor, i, i)
      expectMsg(ContainsResult(i, true))
    }
